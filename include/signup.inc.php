...
if(isset($_POST['signup-submit'])) {
	...
	if(empty($username) || empty($email) || empty($name) || empty($password) || empty($passwordRepeat)){
		header("Location: ../signup?error=emptyfields&uid=".$username."&mail=".$email."&unid=".$name);
		exit();
	}

	else if(!filter_var($email, FILTER_VALIDATE_EMAIL) && !preg_match("/^[a-zA-Z0-9_.]*$/", $username)){
		header("Location: ../signup?error=invalidemailuid");
		exit();
	}

	else if(!filter_var($email, FILTER_VALIDATE_EMAIL)){
		header("Location: ../signup?error=invalidemail&uid=".$username);
		exit();
	}
		...
		if(!mysqli_stmt_prepare($stmt, $sql)){
			...
			if($resultCheck > 0){
				...
				if(!mysqli_stmt_prepare($stmt, $sql)){
					...
					$resultCheck = mysqli_stmt_num_rows($stmt);
					if($resultCheck > 0){
						header("Location: ../signup?error=emailtaken&uid=".$username);
						exit();
					}
					else{
						$sql = "INSERT INTO users (uidUsers, unidUsers, emailUsers, pwdUsers) VALUES (?, ?, ?, ?)";
						$stmt = mysqli_stmt_init($conn);
						if(!mysqli_stmt_prepare($stmt, $sql)){
							header("Location: ../signup?error=sqlerror");
							exit();
